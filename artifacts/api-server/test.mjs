...
{
  // Inline escHtml for unit testing (mirrors the server implementation)
  function escHtml(s) {
    return s
      .replace(/&/g, "&amp;")
      .replace(/</g, "&lt;")
  ...
  ok("< escaped",              escHtml("a<b>c") === "a&lt;b&gt;c");
  ok("double quote escaped",   escHtml('"val"') === "&quot;val&quot;");
  ok("repo name with angle",   escHtml("user/<evil>") === "user/&lt;evil&gt;");
  ok("full injection string",  escHtml('</b><script>alert(1)</script>') ===
     "&lt;/b&gt;&lt;script&gt;alert(1)&lt;/script&gt;");
}

// ─────────────────────────────────────────────────────────────────────────────
console.log("\n🔧  [2] Unit — severity classifier");
{
...
// ─────────────────────────────────────────────────────────────────────────────
console.log("\n🌐  [6] GET /api/github/search — no token");
{
  ...
  ok("reason field present",            typeof body.reason === "string");
  ok("reason mentions GITHUB_TOKEN",    body.reason?.includes("GITHUB_TOKEN"));
}

// ─────────────────────────────────────────────────────────────────────────────
console.log("\n🌐  [7] GET /api/github/search — missing query param");
{
  const { status, body } = await get("/api/github/search");
  ok("HTTP 400 on missing q",           status === 400);
  ok("error mentions q param",          body.error?.toLowerCase().includes("q"));
}

// ─────────────────────────────────────────────────────────────────────────────
console.log("\n🌐  [8] GET /api/github/search — empty query");
{
  const { status, body } = await get("/api/github/search?q=");
  ok("HTTP 400 on empty q",             status === 400);
}
...
// ─────────────────────────────────────────────────────────────────────────────
console.log("\n🌐  [10] POST /api/autoscan/run-now — no token");
{
...
// ─────────────────────────────────────────────────────────────────────────────
console.log("\n🌐  [11] POST /api/github/notify-test — no Telegram config");
{
...
console.log("\n🌐  [12] Static UI — GET /");
{
  ...
  ok("Contains script tag",             html.includes("<script>"));
  ok("Contains esc() function",         html.includes("function esc("));
}

// ─────────────────────────────────────────────────────────────────────────────
console.log("\n📊  [13] TokenPool — response shape validation");
{
  const { body: status } = await get("/api/autoscan/status");
  const pool = status.tokenPool;
  ok("tokenPool is array",              Array.isArray(pool));
  // If tokens exist, validate their shape
  if (pool.length > 0) {
    const t = pool[0];
    ok("token has index",               typeof t.index === "number");
    ok("token has suffix",              typeof t.suffix === "string" && t.suffix.startsWith("..."));
    ...
    ok("no tokens configured (expected in test env)", true);
  }
}
...
console.log("\n📊  [14] Concurrent requests — server stability");
{
  ...
  const allOk = requests.every(r => r.status >= 200 && r.status < 600);
  ok("All 5 concurrent requests returned valid HTTP codes", allOk,
     requests.map(r => r.status).join(", "));
  ok("Config still returns 200",        requests[0].status === 200);
  ok("Autoscan still returns 200",      requests[1].status === 200);
}
